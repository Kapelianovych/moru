...
import { mock, suite, test, type Mock } from "node:test";

import { compile } from "./compiler.js";
import type { Options } from "../src/options.js";
import type { VirtualFile } from "../src/virtual-file.js";

function hasUrlResolverCalledWith(
  urls: Array<string>,
  resolver: Mock<Options["resolveUrl"]>,
): boolean {
...
suite("rebaseUrl", () => {
  test('"HTML component imports should be rebased"', async () => {
    const resolveUrl = mock.fn((file: VirtualFile, url: string) => "test");
    await compile(
      `
